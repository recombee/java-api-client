...
public class DeleteUserPropertyTest extends RecombeeTestCase {
    ...
    public void testDeleteUserProperty() throws ApiException {
        ...
        resp = this.client.send(new DeleteUserProperty("int_property"));
        try {
            this.client.send(new DeleteUserProperty("int_property"));
        ...
        try {
            this.client.send(new DeleteUserProperty("***not_valid$$$"));
            fail("No exception thrown");
        } catch (ResponseException ex) {
        ...
        try {
            this.client.send(new DeleteUserProperty("not_existing"));
            fail("No exception thrown");
        } catch (ResponseException ex) {
            assertEquals(404,ex.getStatusCode());
        }
