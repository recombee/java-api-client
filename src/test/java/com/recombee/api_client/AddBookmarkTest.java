...
public class AddBookmarkTest extends RecombeeTestCase {
    ...
    public void testAddBookmark() throws ApiException {
        ...
        req = new AddBookmark("u_id","i_id").setCascadeCreate(true).setAdditionalData(new HashMap<String, Object>(){{put("answer",42);}});
        resp = this.client.send(req);
        // it 'does not fail with existing item and user'
        req = new AddBookmark("entity_id","entity_id");
        resp = this.client.send(req);
        ...
        req = new AddBookmark("entity_id","entity_id").setTimestamp(new Date(-15));
        try {
            this.client.send(req);
            fail("No exception thrown");
        } catch (ResponseException ex) {
        ...
        req = new AddBookmark("u_id2","i_id2").setCascadeCreate(true).setTimestamp(new Date(5));
        resp = this.client.send(req);
        try {
            this.client.send(req);
            fail("No exception thrown");
